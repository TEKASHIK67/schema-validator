...
├── sitemap_parser.py       — сбор списка URL со страниц отелей через sitemap.xml сайта
├── requirements.txt
├── Dockerfile
...
## Сбор списка URL со страниц отелей (sitemap_parser.py)

Сайт использует двухуровневый sitemap: индекс `sitemap.xml`, который
ссылается на несколько файлов `Part0.xml`, `Part1.xml`, ... — и уже в них
лежат реальные URL страниц. `sitemap_parser.py` сам разбирает эту структуру
и собирает нужное число ссылок на страницы отелей:

```bash
python3 sitemap_parser.py --output urls_hotels.txt --limit 1000
```

Результат — готовый `urls_hotels.txt`, который можно сразу передать в
`cli.py --input`.
